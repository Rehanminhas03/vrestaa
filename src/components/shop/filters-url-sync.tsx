...
import { useFiltersStore, type SortKey } from "@/store/filters";
 ...
 * - On mount: seeds the store from current ?searchParams (browser → store).
 * - After mount: pushes store changes back into the URL via router.replace
 *   (store → browser, no history entry per toggle).
 *
...
export function FiltersUrlSync() {
  ...
  React.useEffect(() => {
    ...
    // Intentional URL → store sync on mount. Guarded by `hydrated.current` so
